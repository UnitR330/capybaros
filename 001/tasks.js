...
function countCows(farmArray) {
  ...
  const numberOfCows = countCows(farm);
  console.log(`Number of cows in the farm: ${numberOfCows}`);
...
function TotalCowAge(farmArray) {
    let totalCowAge = 0;
    for (let i = 0; i < farmArray.length; i++) {
      if (farmArray[i].name === 'Cow') {
        totalCowAge += farmArray[i].age;
      }
    }
    return totalCowAge;
  }
  const totalCowAge = TotalCowAge(farm2);
  console.log(`Total age of cows in the farm: ${totalCowAge}`);
